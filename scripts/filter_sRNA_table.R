...
write_tsv(sRNA_Bc %>% filter(TOTAL_COUNT > 10),"results/Botrytis_matchAligned.min10.tsv,gz")
write_tsv(sRNA_Bc %>% filter(TOTAL_COUNT > 100),"results/Botrytis_matchAligned.min100.tsv.gz")
write_tsv(sRNA_Bc %>% filter(TOTAL_COUNT > 500),"results/Botrytis_matchAligned.min500.tsv.gz")


sRNA_To <- vroom("results/Tomato_matchAligned.out.reads.tsv") %>% filter(LENGTH > 18)

write_tsv(sRNA_To %>% filter(TOTAL_COUNT > 10),"results/Tomato_matchAligned.min10.tsv.gz")
write_tsv(sRNA_To %>% filter(TOTAL_COUNT > 100),"results/Tomato_matchAligned.min100.tsv.gz")
...
TomNoBcInBcOnly <- sRNA_To_mockAdj %>% filter( SumBcOnly <= 10 & SumMockTo > 0) %>% arrange(desc(TOTAL_COUNT))
