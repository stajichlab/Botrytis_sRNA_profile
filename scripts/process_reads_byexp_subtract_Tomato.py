...
with gzip.open(args.botrytis, "r") as bc,  gzip.open(args.tomato, "r") as sl, gzip.open(outsave,'w') as outB, open(outsavebedF,'w') as outBedF, open(outsavebedR,'w') as outBedR:
    ...
    for row in slreader:
        ...
        if name in slhits:
            ...
            slhits.add(name)
    logging.debug("Done processing {}".format(args.tomato))
...
logging.debug("Done processing {}".format(args.botrytis))
...
for i in readsInFeaturesF:
    ...
    for expr in exprnames:
        ect = expr_count[name][expr]
        if len not in sizeprofileexpr[expr]:
            sizeprofileexpr[expr][len] = {TypeClass: { read_feature_direction: ect} }
...
for desc in ['sense', 'antisense', 'combined']:
    ...
    with open(os.path.join(args.outdir,"Botrytis_size_{}_profile.tsv".format(desc)),"w") as ofh:
